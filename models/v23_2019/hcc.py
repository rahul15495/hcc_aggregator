...
import os
import pandas as pd
from models.v23_2019.regvars import *
...
sepsis_pressure_ulcer, sepsis_artif_openings,art_openings_press_ulcer, diabetes_chf,
gcopdcf_asp_spec_b_pneum,asp_spec_b_pneum_pres_ulc, sepsis_asp_spec_bact_pneum,
...
chf_gcopdcf,gsubstanceabuse_gpsychiatric_v23, gcopdcf_card_resp_fail, hcc47_gcancer,hcc85_grenal_v23,hcc85_gcopdcf,
...
def load_diagnostic_category_facts():
	diagnostic_categories = [
					...
					("chf",["85"]),
					("renal_v23",["134","135","136","137","138"]),
					("pressure_ulcer",["157","158"]),
					("sepsis",["2"]),
					("gcopdcf", ["110", "111", "112"]),
					("gsubstanceabuse_v23", ["54","55","56"]),
					("gpsychiatric_v23", ["57","58","59", "60"]) ]
...
def load_rules():
	...
	#chf_renal_v23(CC,CC2) <= dc("chf",CC) & dc("renal_v23",CC2)
	...
	hcc85_grenal_v23(CC,"85") <= dc("renal_v23",CC)


	sepsis_pressure_ulcer(CC,CC2) <= dc("sepsis",CC) & dc("pressure_ulcer",CC2) 
	sepsis_artif_openings(CC,"188") <= dc("sepsis",CC)
	art_openings_press_ulcer(CC,"188") <= dc("pressure_ulcer",CC)
	diabetes_chf(CC,CC2) <= dc("diabetes",CC) & dc("chf",CC2)
	gcopdcf_asp_spec_b_pneum(CC,"114") <= dc("gcopdcf",CC)
	asp_spec_b_pneum_pres_ulc(CC,"114") <= dc("pressure_ulcer",CC)
	...
	indicator(B,'ART_OPENINGS_PRESS_ULCER') <=  ben_hcc(B,CC) & ben_hcc(B,CC2) & art_openings_press_ulcer(CC,CC2)
	indicator(B,'ASP_SPEC_B_PNEUM_PRES_ULC') <=  ben_hcc(B,CC) & ben_hcc(B,CC2) & asp_spec_b_pneum_pres_ulc(CC,CC2)
	...
	#indicator(B,'CHF_RENAL') <=  ben_hcc(B,CC) & ben_hcc(B,CC2) & chf_renal_v23(CC,CC2)
	indicator(B,"gCopdCF_ASP_SPEC_B_PNEUM") <=  ben_hcc(B,CC) & ben_hcc(B,CC2) & gcopdcf_asp_spec_b_pneum(CC,CC2)
	...
	#indicator(B,"gSubstanceAbuse_gPsychiatric_v23") <=  ben_hcc(B,CC) & ben_hcc(B,CC2) & gsubstanceabuse_gpsychiatric_v23(CC,CC2)
	indicator(B,'HCC85_HCC96') <=  ben_hcc(B,'85')  & ben_hcc(B,'96')

	indicator(B,"disable_substAbuse_psych_V23") <= (age_range(B,0,64)) & (ben_hcc(B,CC) & ben_hcc(B,CC2) & gsubstanceabuse_gpsychiatric_v23(CC,CC2))
	...
	indicator(B,'HCC85_gRenal_V23') <=  ben_hcc(B,CC) & ben_hcc(B,CC2) & hcc85_grenal_v23(CC,CC2)
